# MaalFlow — System States and Feedback

**Status:** CANONICAL · **Created:** 2026-08-26 (expanded from
`03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md` §10 into its own document, per the
...
be shown (rare — most MaalFlow screens have a known layout).
...
- **Network/offline:** MaalFlow is local-first — most screens should keep
...
- **Destructive-operation error:** distinct from a normal error — pairs with
  the `financial.danger` token (see `04_MAALFLOW_DESIGN_TOKEN_CATALOG.md`).
- **Data-integrity error:** surfaced honestly, never silently repaired —
  consistent with the standing financial-safety rule that MaalFlow never
...
Given MaalFlow's local-first architecture, "offline" is not a global blocking
...
`02_MAALFLOW_VISUAL_IDENTITY_CANONICAL.md`.
...
MaalFlow's restore flow may show a neutral status indicator (e.g. "restoring…",
