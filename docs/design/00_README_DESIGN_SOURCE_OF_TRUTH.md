...
**`01_MAALFLOW_MASTER_PRODUCT_DESIGN_BLUEPRINT.md`** — the highest-authority
...
| Topic | Document |
|---|---|
| Visual identity, brand, color governance | `02_MAALFLOW_VISUAL_IDENTITY_CANONICAL.md` |
| Components, interaction rules (not navigation) | `03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md` |
| Exact token values (colors, type, spacing, radius) | `04_MAALFLOW_DESIGN_TOKEN_CATALOG.md` |
| Component inventory, migration, legacy-screen policy | `05_MAALFLOW_COMPONENT_ARCHITECTURE.md` |
| Navigation, IA, destination map, Add Method, Settings root | `06_MAALFLOW_NAVIGATION_AND_INFORMATION_ARCHITECTURE.md` |
| Per-screen specifications | `07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md` |
| Process/governance — reuse rules, checklists, decision template | `08_MAALFLOW_DESIGN_GOVERNANCE.md` |
| Accessibility, RTL, responsive (confirmed vs. validation-pending) | `09_MAALFLOW_ACCESSIBILITY_RTL_AND_RESPONSIVE.md` |
| Reports/chart taxonomy and rules | `10_MAALFLOW_CHART_AND_FINANCIAL_VISUALIZATION_SYSTEM.md` |
| Loading/empty/error/success/sync-status presentation | `11_MAALFLOW_SYSTEM_STATES_AND_FEEDBACK.md` |
| Implementation sequencing (not authorization) | `12_MAALFLOW_DESIGN_MIGRATION_ROADMAP.md` |
| Chronological record of every design decision | `13_MAALFLOW_DESIGN_DECISION_LOG.md` |
| Approved mockups — what they approve, what's an artifact | `14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md` |
...
- **The Frozen Master Plan and engineering phase order**
  (`docs/01_CORE_AUTHORITY/MAALFLOW_MASTER_PLAN_FROZEN.md`) — this design set
  does not renumber or replace engineering phases.
- **Financial contracts, schema, migration, backup/restore, sync, security**
  (`docs/MAALFLOW_FINANCIAL_CONTRACT.md`, `docs/MAALFLOW_SECURITY_THREAT_MODEL.md`,
...
- **The original 2026-08-25 Product Owner blueprint text**
  ...
  — superseded for day-to-day reference by `01_MAALFLOW_MASTER_PRODUCT_DESIGN_BLUEPRINT.md`
...
## Authority order (mirrors `docs/00_MAALFLOW_CANONICAL_AUTHORITY.md`)

1. Actual current repository/runtime evidence.
2. Latest explicit Product Owner decision (text overrides an image
   artifact — see `08_MAALFLOW_DESIGN_GOVERNANCE.md` §2).
...
5. Older design/product documentation (`docs/MAALFLOW_UI_REDESIGN_SPEC_AR.md`).
...
device feedback loop (`01_MAALFLOW_MASTER_PRODUCT_DESIGN_BLUEPRINT.md` §18), not
