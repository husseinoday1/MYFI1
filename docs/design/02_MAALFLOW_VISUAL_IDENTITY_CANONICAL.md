# MaalFlow — Canonical Visual Identity
...
(current-vs-target evidence), `docs/design/14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md`
...
This document defines what MaalFlow should *feel like*. `03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md`
defines the components that deliver it, and `04_MAALFLOW_DESIGN_TOKEN_CATALOG.md`
defines the exact values.

## 1. Design philosophy

MaalFlow is a **local-first, financially trustworthy** tool, not a lifestyle app.
...
**MaalFlow must not feel:** futuristic, neon, gaming-like, crypto-like, visually
...
This is a governance/architecture requirement (see `04_MAALFLOW_DESIGN_TOKEN_CATALOG.md`
...
is now in `04_MAALFLOW_DESIGN_TOKEN_CATALOG.md` §Category tokens (approved
...
(`MaalFlow-Cairo`, applied globally via `src/lib/fonts.js`). This is the
canonical scale — see `04_MAALFLOW_DESIGN_TOKEN_CATALOG.md` for exact values. The
...
task (`05_MAALFLOW_COMPONENT_ARCHITECTURE.md`), not a new typography
decision.
...
`03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md` as a proposal, not a confirmed target.
...
- Adding a payment-method/card/bank, VAT, or rounding-method settings surface.
  **Status: REJECTED FOR CURRENT TARGET (2026-08-26).** REF-07's "طرق الدفع"
  and VAT/rounding rows are not approved MaalFlow capabilities and must not be
