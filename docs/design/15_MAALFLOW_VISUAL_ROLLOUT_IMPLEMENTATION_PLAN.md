# MaalFlow — خطة تنفيذ الهوية البصرية الجديدة

**الحالة:** معتمدة من مالك المنتج · **التاريخ:** 2026-08-28
**الهدف:** تحويل معاينات MaalFlow المعتمدة إلى واجهات حقيقية متماسكة في التطبيق، على مراحل صغيرة وآمنة، من دون تعطيل منطق المال أو تغيير البيانات أو رفع أي تغييرات إلى GitHub قبل طلب صريح.
...
├─ البيانات والملفات
├─ المزايا: بصيرة MaalFlow والمكافآت
...
6. مدخل واحد إلى «بصيرة MaalFlow».

لا تتحول هذه الصفحة إلى تبويبات كثيرة أو أدوات مقارنة معقدة.

### بصيرة MaalFlow — التحليل المتقدم

اسم المنتج المعتمد: **بصيرة MaalFlow**.
...
### المرحلة 4 — التقارير وبصيرة MaalFlow (مسؤول Codex · قيد الإكمال محليًا)

**النطاق:** `ReportsScreen` وعقود العرض/التصدير المتعلقة به فقط.

- إعادة بناء التقرير العادي وفق §3.
- نقل المقارنة الحرة والاتجاهات والتفصيل وPDF إلى «بصيرة MaalFlow».
...
- «المزيد» مركز تنقل منظم؛ لا مفاتيح تشغيل داخل الصفحة الأم. فيه إدارة
  المحافظ والتصنيفات والأرشيف والاشتراكات والمزايا والبيانات والمساعدة.
...
- خطة توزيع الدخل تُفتح من «أموالي» كبوابة مستقلة؛ تحفظ نسبًا صالحة مجموعها
  100%، وتحولها إلى مبالغ، وتظهر نتيجتها لاحقًا في التقرير.

**معيار القبول:** لا إعداد وهمي؛ التخطيط محفوظ ومقاس مقابل الواقع، والمسارات قابلة للوصول من «المزيد».

### المرحلة 6 — التدويل ودعم اللغات

**الهدف:** نقل MaalFlow من نصوص عربية/إنكليزية موزعة داخل الشاشات إلى نظام ترجمة مركزي قابل للتوسع، من دون إظهار أي لغة للمستخدم قبل اكتمالها.

- اللغتان المكتملتان حاليًا هما العربية والإنكليزية، ويجب الحفاظ على تكافؤهما في كل شاشة ورسالة خطأ وحالة فارغة وإشعار.
- أولوية التوسع المقترحة: الكردية السورانية (`ckb`) أولًا لملاءمة السوق العراقي، ثم التركية (`tr`) والفارسية (`fa`)، ثم الفرنسية (`fr`) بعد اكتمال الأساس. ترتيب الإطلاق النهائي يثبت بقرار المنتج وقياس السوق، ولا تُعرض لغة جزئية في الإعدادات.
- إنشاء قاموس ترجمة مركزي بمفاتيح دلالية، وإزالة النصوص المباشرة من المكونات تدريجيًا، مع fallback واضح إلى الإنكليزية عند غياب مفتاح أثناء التطوير فقط.
- دعم اتجاه كل لغة من بيانات locale نفسها: العربية والكردية السورانية والفارسية RTL؛ الإنكليزية والتركية والفرنسية LTR. لا يُستنتج الاتجاه من اختيار الدولة.
- توحيد تنسيق الأرقام، الفواصل العشرية، العملات، التواريخ، صيغ الجمع، وترتيب النص المختلط بحسب اللغة والمنطقة، مع بقاء القيمة المالية المخزنة مستقلة عن طريقة عرضها.
- يبقى اختيار لغة شاشة الترحيب معاينة للأونبوردنغ فقط، بينما اختيار اللغة في Essentials/الإعدادات هو التفضيل المحفوظ للتطبيق كله.
- إضافة فحص اكتمال يمنع شحن لغة ناقصة، واختبارات تبديل حي للغة، RTL/LTR، النصوص الطويلة، قارئ الشاشة، ولقطات داكن/فاتح لكل لغة مدعومة.

**معيار القبول:** لا نص مضمّن خارج طبقة الترجمة في المسارات المشمولة، تغطية 100% للمفاتيح الإلزامية، لا اختلاط لغتين في الشاشة، واتجاه/أرقام/تواريخ صحيحة على جهاز فعلي قبل تفعيل اللغة للمستخدم.

### المرحلة 7 — الدمج والتحقق
...
### مرحلة مستقبلية 17-G — صانع المتابعات المالية

هذه ليست تغييرًا بصريًا سريعًا ولا تُدمج مع المرحلة 3. «صانع المتابعات» سيكون الموطن الوحيد لإنشاء وتعديل وحذف **أنواع المتابعات المالية** من `المزيد ← أدواتي`؛ أما بوابة المتابعات فتعرض الأنواع والعناصر والنتائج فقط. يستطيع المستخدم بناء نوع مثل «أقساط البيت» وتحديد مصادره (محافظ/حساب/فئات/طرف)، سلوكه المالي، وواجهة دفعه قبل حفظه. القسط والاشتراك يصبحان قوالب من المحرك بعد الترحيل الآمن. لا تبدأ الواجهة أو المحرك قبل عقد بيانات، Ledger V7، وخطة تكافؤ للهياكل الحالية. التصميم والبحث وخطوات 17-G موثقة في [MYFI_FINANCIAL_TRACKER_BUILDER_FUTURE_DESIGN_2026-08-30.md](../04_CURRENT_EVIDENCE/MYFI_FINANCIAL_TRACKER_BUILDER_FUTURE_DESIGN_2026-08-30.md).

### مرحلتان مستقبليتان 17-C و17-E — MaalFlow Flow وبصيرة MaalFlow

`17-C` يحوّل توزيع الدخل من نسب عامة إلى خطة حية مرتبطة بفئات وميزانيات وأهداف والتزامات، عبر معاينة صريحة قبل أي حفظ. `17-E` يبني بصيرة MaalFlow كمساحة للمقارنة المفتوحة والاتجاهات والأسباب والتوقعات القابلة للتدقيق، ثم مساعد محادثي للقراءة فقط بعد موافقة خصوصية مستقلة. لا تُخفى المقارنة أو مخططها؛ التقرير العادي يحتفظ بمقارنة صغيرة، وتظهر المقارنة الحرة والمخطط بوضوح في بصيرة. عقد المنتج والمصادر وخريطة التنفيذ موثقة في [MYFI_PLAN_ENGINE_AND_REPORTS_PRODUCT_DIRECTION_2026-08-30.md](../04_CURRENT_EVIDENCE/MYFI_PLAN_ENGINE_AND_REPORTS_PRODUCT_DIRECTION_2026-08-30.md).
...
> أنت تعمل بالتوازي مع Codex على مشروع MaalFlow. اقرأ كاملًا أولًا: `docs/design/00_README_DESIGN_SOURCE_OF_TRUTH.md`، `docs/design/02_MAALFLOW_VISUAL_IDENTITY_CANONICAL.md`، `docs/design/07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md`، وهذا الملف `docs/design/15_MAALFLOW_VISUAL_ROLLOUT_IMPLEMENTATION_PLAN.md`. راجع كذلك الصور المرجعية في `docs/design/assets/visual-rollout-2026-08-28/` قبل أي كود.
>
...
| التقرير العادي | بصيرة MaalFlow | توزيع الدخل |
|---|---|---|
| ![التقرير العادي](assets/visual-rollout-2026-08-28/01-reports-standard-dark.png) | ![بصيرة MaalFlow](assets/visual-rollout-2026-08-28/02-basira-advanced-dark.png) | ![توزيع الدخل](assets/visual-rollout-2026-08-28/03-income-allocation-dark.png) |
...
- `docs/design/01_MAALFLOW_MASTER_PRODUCT_DESIGN_BLUEPRINT.md`
- `docs/design/02_MAALFLOW_VISUAL_IDENTITY_CANONICAL.md`
- `docs/design/06_MAALFLOW_NAVIGATION_AND_INFORMATION_ARCHITECTURE.md`
- `docs/design/07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md`
- `docs/design/09_MAALFLOW_ACCESSIBILITY_RTL_AND_RESPONSIVE.md`
- `docs/design/10_MAALFLOW_CHART_AND_FINANCIAL_VISUALIZATION_SYSTEM.md`
