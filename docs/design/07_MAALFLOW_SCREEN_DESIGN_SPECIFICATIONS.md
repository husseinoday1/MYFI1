# MaalFlow — Screen Design Specifications
...
`14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md`, `06_MAALFLOW_NAVIGATION_AND_INFORMATION_ARCHITECTURE.md`.
...
- **Required sections:** per-step content defined in
  `14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md` (REF-02/03/03B/03D/03C/03E).
...
- **Approved structure:** centered MaalFlow header (profile left, bell right);
...
- **Quick Add behavior:** exactly as defined in
  `06_MAALFLOW_NAVIGATION_AND_INFORMATION_ARCHITECTURE.md` §4 — global setting,
...
- **Empty state:** a fresh install's Home (no transactions yet) —
  **UNKNOWN** exact current copy/behavior, but must follow the "explain what
  + why + next action" rule in `11_MAALFLOW_SYSTEM_STATES_AND_FEEDBACK.md`.
- **Loading state:** skeleton, per `11_MAALFLOW_SYSTEM_STATES_AND_FEEDBACK.md`.
...
- **Approved structure (revised 2026-08-29 by Product Owner):** five
  ...
  allocation plan, 4) Reports, 5) MaalFlow Basira. Each opens one real
...
- **Empty/loading/error:** per `11_MAALFLOW_SYSTEM_STATES_AND_FEEDBACK.md`;
...
- **Chart rules:** per `10_MAALFLOW_CHART_AND_FINANCIAL_VISUALIZATION_SYSTEM.md`.
...
## MaalFlow Basira (My Money gateway 5)
...
- **Approved structure:** My Shortcuts (exactly 3, user-customizable); My
  Tools; Data & Files; Benefits/Rewards; MaalFlow & Help; Settings.
...
- **Adaptive-More governance:** PERMANENT / TEMPORARY-MATURITY-DEPENDENT /
  CONDITIONAL classification per `08_MAALFLOW_DESIGN_GOVERNANCE.md`.
- **Financial-data impact:** NONE.
...
- **Approved structure:** exactly 5 root sections (Account & Sync,
  ...
  `06_MAALFLOW_NAVIGATION_AND_INFORMATION_ARCHITECTURE.md` §5.
...
`05_MAALFLOW_COMPONENT_ARCHITECTURE.md` for their retirement policy.
