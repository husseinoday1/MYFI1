# MaalFlow — Master Product Design Blueprint
...
`docs/00_MAALFLOW_CANONICAL_AUTHORITY.md`; this document is what should now be
...
MaalFlow is a **local-first personal finance app**. On-device SQLite is the
...
delight-through-decoration. Full detail: `02_MAALFLOW_VISUAL_IDENTITY_CANONICAL.md`.

## 3. Product positioning (corrected 2026-08-26)

MaalFlow is **not** defined as a product for Arab users only. Arabic-first and
...
`14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md`, REF-02.
...
Settings. Full navigation rules and rationale: `06_MAALFLOW_NAVIGATION_AND_INFORMATION_ARCHITECTURE.md`.

### 4.2 Home — locked product contract

Answers: *"Where do I stand now?"* Centered MaalFlow identity/header; total
...
Full spec: `07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md`.
...
`07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md`.
...
spec: `07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md`.
...
Rewards** · **MaalFlow & Help** · **Settings**. Adaptive-More governance
...
One global setting at **More → My Tools → Customize MaalFlow → Add Method**,
...
it belongs to the MaalFlow olive/green brand system; a generated reference
...
`04_MAALFLOW_DESIGN_TOKEN_CATALOG.md`). Full presentation rules:
`10_MAALFLOW_CHART_AND_FINANCIAL_VISUALIZATION_SYSTEM.md`.
...
This document defines *what* MaalFlow is structurally; `03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md`
defines the components and interaction rules that deliver it;
`04_MAALFLOW_DESIGN_TOKEN_CATALOG.md` defines exact values. None of the three
...
`02_MAALFLOW_VISUAL_IDENTITY_CANONICAL.md` §6 and `09_MAALFLOW_ACCESSIBILITY_RTL_AND_RESPONSIVE.md`.
...
canonical. Full detail: `09_MAALFLOW_ACCESSIBILITY_RTL_AND_RESPONSIVE.md`.

## 11. Accessibility

Full detail: `09_MAALFLOW_ACCESSIBILITY_RTL_AND_RESPONSIVE.md`. Confirmed rules
...
`11_MAALFLOW_SYSTEM_STATES_AND_FEEDBACK.md`. This document does not describe or
...
Action), and chart-system rules: `10_MAALFLOW_CHART_AND_FINANCIAL_VISUALIZATION_SYSTEM.md`.
...
intentional, not drift. Governance: `08_MAALFLOW_DESIGN_GOVERNANCE.md`.
...
and the six-step removal gate: `08_MAALFLOW_DESIGN_GOVERNANCE.md` and
`05_MAALFLOW_COMPONENT_ARCHITECTURE.md`.
...
`12_MAALFLOW_DESIGN_MIGRATION_ROADMAP.md`.
