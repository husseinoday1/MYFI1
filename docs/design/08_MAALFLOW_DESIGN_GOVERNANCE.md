# MaalFlow — Design Governance

**Registered:** 2026-08-25 · **Status:** CANONICAL

This document defines how MaalFlow stays visually coherent after the current
...
5. `docs/design/14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md`.
6. Older MaalFlow design/product documents (`MAALFLOW_UI_REDESIGN_SPEC_AR.md`, etc.).
...
order in `docs/00_MAALFLOW_CANONICAL_AUTHORITY.md`; this section is the
...
confirmed instances exist today (see `14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md`,
...
- Which tokens does it use (`04_MAALFLOW_DESIGN_TOKEN_CATALOG.md`)?
- Which existing primitive(s) does it reuse (`03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md`,
...
- A new primitive requires an entry added to `05_MAALFLOW_COMPONENT_ARCHITECTURE.md`
  before or alongside its implementation — no primitive should exist in code
  without a corresponding inventory row.
- A new token requires an entry in `04_MAALFLOW_DESIGN_TOKEN_CATALOG.md` with Light
  and Dark values before use in more than one screen.
- A new screen requires a row in `07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md`.
...
`03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md`.
