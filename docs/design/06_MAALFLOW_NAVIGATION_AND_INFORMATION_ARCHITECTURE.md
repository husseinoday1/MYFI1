# MaalFlow — Navigation and Information Architecture

**Status:** CANONICAL · **Consolidated:** 2026-08-26 (extracted from
`03_MAALFLOW_DESIGN_SYSTEM_CANONICAL.md` §2 into its own document, per the
Master Blueprint rebuild, to separate IA authority from component/
interaction authority)
**Basis:** `01_MAALFLOW_MASTER_PRODUCT_DESIGN_BLUEPRINT.md` §4,
`14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md` (REF-04's on-canvas
...
logic already exist (see `07_MAALFLOW_SCREEN_DESIGN_SPECIFICATIONS.md`).
...
| **More** | Primary tab | Bottom nav | My Shortcuts, My Tools, Data & Files, Benefits, MaalFlow & Help, Settings |
...
**Location:** More → My Tools → Customize MaalFlow → Add Method.
...
canonically purple; it belongs to the MaalFlow olive/green brand system.
...
for the current target — see `14_MAALFLOW_APPROVED_VISUAL_REFERENCE_REGISTER.md`,
REF-07.
...
1. Welcome with a compact AR/EN language toggle in the top side of the screen
header → 2–4. Three short personalization questions → 5. Essentials (app
language, country, currency, appearance, first-wallet name) and start. The
Welcome control previews onboarding copy and RTL/LTR immediately; the
language row in Essentials is the explicit app-wide preference stored on
completion.
