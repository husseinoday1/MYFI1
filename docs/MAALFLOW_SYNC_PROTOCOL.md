# MaalFlow — Sync Protocol Contract

## الدور

Cloud هو replica/transport، وLocal SQLite commit هو البداية المالية.

## تقليل بيانات السحابة — قاعدة دائمة

السحابة ليست نسخة من إعدادات الهاتف. أي حقل جديد يبقى محلياً افتراضياً؛ لا
يُضاف إلى البيانات السحابية إلا بسبب مالي واضح، وبعد مراجعة واختبار لقائمة
السماح في `src/lib/cloudWorkspaceMetadata.js`.

- **يُشارك:** السجل المالي اللازم للمزامنة متعددة الأجهزة (الحركات وقيمها
  ووصفها/ملاحظاتها، المحافظ والتصنيفات، الديون والأهداف والالتزامات والميزانيات
  والقواعد المتكررة، العملات وأسعار الصرف، ومعرّفات المزامنة/المراجعات/الأجهزة
  وحقبة الاستعادة). من إعداد مساحة العمل لا يُشارك إلا `currency`.
- **هوية الحساب منفصلة:** البريد والجلسة تديرهما Supabase Auth. الاسم واسم
  المستخدم ورقم الهاتف ومسار/صورة الحساب لا تذهب إلى السجل المالي؛ تذهب فقط إلى
  `profiles`/Storage عند إجراء حساب صريح يتطلبها.
- **محلي فقط:** اللغة والثيم والتدوير وصيغة التاريخ والدولة، ترتيب الشاشة
  والبطاقات والإجراءات، إظهار الأرصدة، وضع الإدخال، البداية الافتراضية، المحفظة
  الافتراضية، نطاق/نوع العرض، الوحدات المرئية، القفل الحيوي، قبول الشروط،
  وتفضيلات الإشعارات. هذه لا تشغّل مزامنة ولا تُرسل كـworkspace mutation.

أي استثناء يحتاج سبباً مالياً مكتوباً، قائمة سماح محدثة، واختبار يمنع تسريب
الحقول المحلية مستقبلاً.
