...
# MaalFlow — MASTER ENGINEERING EXECUTION PLAN

## الخطة الهندسية والتنفيذية النهائية الشاملة

**الحالة:** Frozen Master Plan بعد الاعتماد
**النطاق:** MaalFlow Current Real State
**المبدأ:** لا Rewrite من الصفر، ولا Feature جديدة فوق أساس مالي غير مثبت
**الهدف:** تحويل MaalFlow من Advanced Beta إلى تطبيق مالي Local-first يمكن الاعتماد عليه لسنوات

---

# 1. الهدف النهائي

MaalFlow يجب أن ينتهي إلى نظام تكون فيه الحقيقة المالية كالتالي:
...
إذا وجد MaalFlow تناقضاً مالياً:
...
## MaalFlow Production Scope الأول
...
```text
docs/MAALFLOW_RELEASE_GATE_STATUS_AR.md
docs/MAALFLOW_DATA_OWNERSHIP.md
docs/MAALFLOW_FINANCIAL_CONTRACT.md
docs/MAALFLOW_DATE_TIME_CONTRACT.md
docs/MAALFLOW_SYNC_PROTOCOL.md
docs/MAALFLOW_MIGRATION_POLICY.md
docs/MAALFLOW_BACKUP_FORMAT.md
docs/MAALFLOW_SECURITY_THREAT_MODEL.md
docs/MAALFLOW_PERFORMANCE_SLO.md
docs/MAALFLOW_RELEASE_SCOPE.md
```
...
إذا MaalFlow لا يدعم multiple ledgers حالياً:
...
# 177. متى نعتبر MaalFlow Near Production؟
...
# 178. متى نعتبر MaalFlow Production Ready؟
...
كل مرحلة من الخطة يجب أن تجعل MaalFlow:
...
* بعد الربط: نفس الاسم والصورة + `متصل بحساب MaalFlow`.
* account optional.
...
هذه البنود نتجت من مراجعة MaalFlow كمستخدمين متعددين ذوي أهداف مختلفة.
...
هذا الملحق يصبح جزءاً من MaalFlow Frozen Master Plan من تاريخ 2026-08-15.
...
هذا التعديل يصبح جزءاً من MaalFlow Frozen Master Plan من تاريخ 2026-09-04.

---

# 197. تدقيق شامل لسلامة المعاملات — Plan Addition, 2026-09-04

## السبب

بلاغ حقيقي من المالك 2026-09-04: حذف حركات مرتبطة بهدف مكتمل ومحوَّل
("released") لم يغيّر حالة الهدف المعروضة إطلاقاً. التحقيق
([[myfi_goal_transaction_integrity_resolved_2026-09-04]]) وجد السبب
(`goalLifecycle` بـ`trackerLifecycle.js` يتجمّد بشكل غير مشروط عند
`status === 'released'`)، أصلحه (تنبيه صادق عند الحذف، بدون تغيير منطق)،
وعمّم الفحص على الديون والأقساط لنفس فئة الخلل بالذات (حالة متجمّدة
تتجاهل حذف الحركات) — كلاهما سليم.

**لكن هذا لم يكن تدقيقاً شاملاً.** كان تحقيقاً مركّزاً على نوع واحد محدد
من الخلل (حالة متجمّدة/مخبّأة لا تُشتق حياً من الحركات الفعلية)، امتد فقط
لنفس الفئة عبر الأهداف/الديون/الأقساط. المالك صرّح صراحة أن المعاملات هي
ثاني أهم ميزة بالتطبيق ("ثاني اهم ميزه بالبرنامج ولا اريدها ان تظرب باي
خلل") وطلب إضافة تدقيق أوسع كمشروع منفصل.

## النطاق المقترح

تدقيق منهجي — ليس بحثاً عن خلل واحد بل مسحاً شاملاً — يغطي:

* كل أنواع الحركات: دخل، صرف، تحويل بين محافظ، دفعات ديون، توفير أهداف،
  دفعات أقساط/اشتراكات، تسويات أرصدة، الرصيد الافتتاحي.
* كل تفاعل بين المعاملات والأنظمة الأخرى: فروقات العملة (FX) التاريخية
  المجمّدة مقابل الحالية، الوسوم (tags)، الأرشفة، المزامنة (sync) —
  خصوصاً بعد أي عملية حذف/تعديل/استرجاع.
* لكل دورة حياة (lifecycle) لكل نوع متابعة (tracker): هل الحالة النهائية
  تُشتق حياً من الحركات الفعلية دائماً، أم يمكن أن تتجمّد/تُخبّأ بعد أي
  حدث "نهائي" (إكمال، إغلاق، تحويل، أرشفة)؟ هذا بالضبط نوع الخلل اللي
  اكتُشف بالأهداف — يحتاج فحص ممنهج، مو ثقة عمياء لأن نوعين آخرين طلعوا
  سليمين هالمرة.
* صحة حسابات الأرصدة (المحافظ، الالتزامات المحجوزة) تحت كل سيناريوهات
  الحذف/التعديل، وليس فقط سيناريو واحد مُبلَّغ عنه.

## المنهجية

نفس انضباط تحقيقات اليوم (Phase 14/15): تتبّع فعلي للكود لا افتراض، تقرير
بصيغة PROVEN/PARTIAL/MISSING لكل نوع حركة وكل تفاعل، مع دليل (اسم ملف/سطر
أو اختبار) لكل نتيجة — لا استنتاج بدون تحقق مباشر.

## الموضع بالخطة

يرتبط منطقياً بـPhase 17 (§115 "Budget Intelligence + Recurring + Product
Correctness" — "Product Correctness" موجودة أصلاً بعنوان المرحلة). لكن
نظراً لأولوية المالك الصريحة والمتكررة لسلامة المعاملات، هذا التدقيق لا
ينتظر بالضرورة بدء Phase 17 الكامل (ميزانية ذكية، تكرار) — يمكن تنفيذه
كعنصر مبكر ومستقل بمجرد توفر جلسة Implementation، بالتوازي مع أي عمل
Phase 15/16 آخر، دون حاجة لانتظار ترتيب المراحل الرسمي.

## الحالة

لم يبدأ. جاهز للتسليم لجلسة Implementation عند توفرها (أداة التنسيق
المباشر معطّلة وقت كتابة هذا القسم — راجع
[[myfi_sync_honesty_pushed_2026-09-04]]).
