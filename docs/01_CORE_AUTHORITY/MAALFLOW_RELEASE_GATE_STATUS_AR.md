...
| P01-SIGN-001 | Production signing ليس debug signing | `android/app/build.gradle`, `android/keystore.properties.example` | in_progress | source audit | Android Gradle | n/a | 1.0.0 | n/a | source tree | `release` يوقّع بالمفتاح الحقيقي إن وُجد `android/keystore.properties` محليًا (git-ignored)، ويسقط إلى debug عند غيابه — لا تغيير على سلوك CI الحالي | لم يُختبر ببناء موقَّع فعلي بعد (لا Android SDK محليًا هنا)، ولا برفعة Play | لا ادعاء Release Ready حتى بناء موقَّع حقيقي مُتحقَّق منه؛ credential path نفسه لم يعد الحاجب | 2026-09-10 |
...
- rotation behavior matched the selected MaalFlow setting.
...
- `P04R1-004 / P18-005` at `ef2f2c6`: MaalFlow decision windows for merge-result and backup restore, corrected action icons, locally pinned controlled EAS CLI build path, and combined gate contract.
