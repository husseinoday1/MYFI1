...
- أدوات التطوير:
  ...
  - إضافة `eas-cli` كاعتماد تطوير حتى يعمل `npm run build:aab` من داخل المشروع.
...
تثبيت أدوات التطوير `supabase@2.113.0` و`eas-cli@21.7.0` أظهر تحذير `npm audit` بوجود 34 مشكلة في شجرة الاعتمادات، مع سكربت اختياري محجوب لـ `dtrace-provider`. لا يتم تشغيل `npm audit fix --force` ضمن هذه الحزمة لأنه قد يغير نسخ Expo/React Native ويحتاج اختبار توافق منفصل.
