...
assert(/function EssentialsSlide\([\s\S]*?onLanguage/.test(onboarding) && onboarding.includes("langMode: 'manual'"), 'Essentials must expose the real app-language choice, committed as a whole-app preference when onboarding finishes');
