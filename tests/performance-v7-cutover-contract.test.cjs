...
must(demoBranch.includes("markStartupStage('performance:fixtureRegenerated')"), 'performance startup does not time full-fixture regeneration after a failed reuse proof');
...
must(demoBranch.includes('buildPerformanceTestWorkspaceAsync'), 'a failed lab reuse proof can rebuild from the bounded UI cache');
must(demoBranch.includes('__performanceArchives: regeneratedArchives'), 'a failed lab reuse proof does not retain the complete regenerated archive source');
must(demoBranch.includes('workspace: regeneratedDemo'), 'a failed lab reuse proof rebuilds from the bounded UI cache instead of the full regenerated fixture');
must(demoBranch.includes('coldArchives: regeneratedArchives'), 'a failed lab reuse proof does not stage the regenerated archived fixture rows');
must(demoBranch.includes('forceReplace: true'), 'a failed lab reuse proof can skip the required deterministic full-fixture rebuild');
must(!demoBranch.includes('workspace: loadedDemo,\n            coldArchives'), 'a failed lab reuse proof still stages the bounded UI cache');
must(!demoBranch.includes('performance:coldArchiveExport'), 'startup still presents a bounded-cache fallback as cold-archive hydration');
...
must(demoBranch.includes('if (performanceLedger?.ok && performanceLedger?.cutover)'), 'a successful rebuilt V7 fixture is not rehydrated through the bounded cache');
must(!demoBranch.includes('if (performanceLedger?.alreadyCutover && performanceLedger?.ok)'), 'only reused fixtures are bounded after startup; freshly rebuilt fixtures still mount every row');
