...
const stepText = (lang) => {
  ...
  return {
    ...
    detectLocation: ar ? 'تحديد الدولة والعملة من موقعي' : 'Detect country and currency from my location',
    detectingLocation: ar ? 'جارٍ تحديد الموقع...' : 'Detecting location...',
    locationHint: ar ? 'اقتراح لمرة واحدة، ويمكنك التغيير يدويًا' : 'A one-time suggestion you can change manually',
    locationDenied: ar ? 'لم تُمنح صلاحية الموقع. يمكنك اختيار الدولة والعملة يدويًا.' : 'Location permission was not granted. You can choose manually.',
    locationUnsupported: ar ? 'تم تحديد الموقع، لكن الدولة غير مدعومة بعد. اختر الدولة والعملة يدويًا.' : 'Location was found, but the country is not supported yet. Choose manually.',
    locationFailed: ar ? 'تعذر تحديد الموقع الآن. حاول مرة أخرى أو اختر يدويًا.' : 'Could not detect location. Try again or choose manually.',
  };
};
...
export default function OnboardingScreen({ cfg, onDone }) {
  ...
  const countryOptions = [
    { value: 'auto', label: lang === 'ar' ? 'تلقائي' : 'Auto', detail: lang === 'ar' ? 'حسب الموقع' : 'From location', icon: 'location-outline', country: null },
  ...
  const detectCountryAndCurrency = async () => {
    ...
    try {
      ...
      Alert.alert('', result.reason === 'unsupported' ? T.locationUnsupported : T.locationDenied);
    } catch {
  ...
  return (
    ...
    >
      ...
      {step === 3 ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.ideaContent}>
          <View style={s.ideaVisual}>
            <View style={[s.ideaGlow, { backgroundColor: `${th.primary}0D` }]} />
            <View style={[s.ideaPhone, { backgroundColor: th.card, borderColor: th.border }]}>
              <View style={[s.ideaPhoneHead, { flexDirection: isAr ? 'row-reverse' : 'row' }]}>
                <View style={[s.ideaMiniLogo, { backgroundColor: th.primary }]}>
                  <Ionicons name="leaf-outline" size={17} color={th.onPrimary} />
                </View>
                <Text style={[s.ideaBrand, { color: th.text }]}>MYFI</Text>
              </View>
              <View style={[s.ideaBalance, { backgroundColor: th.primaryContainer }]}>
                <Text style={[s.ideaBalanceLabel, { color: th.onPrimaryContainer }]}>{isAr ? 'ملخص أموالك' : 'Financial overview'}</Text>
                <View style={[s.ideaBalanceLine, { backgroundColor: th.primary }]} />
                <View style={[s.ideaBars, { flexDirection: isAr ? 'row-reverse' : 'row' }]}>
                  {[38, 68, 48, 82, 58].map((height, index) => (
                    <View key={index} style={[s.ideaBar, { height, backgroundColor: index === 3 ? th.primary : `${th.primary}66` }]} />
                  ))}
                </View>
              </View>
            </View>
          ...
          <View style={s.ideaList}>
            ...
            <ConceptRow th={th} isAr={isAr} icon="share-social-outline" title={T.ideaShare} body={T.ideaShareBody} />
          </View>
        </ScrollView>
      ) : step === 1 ? (
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" contentContainerStyle={[s.content, { paddingBottom: 110 }]}>
