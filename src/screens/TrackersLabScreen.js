...
import { Alert, Modal, ScrollView, View, Text, TextInput, StyleSheet } from 'react-native';
...
export default function TrackersLabScreen({ focusRequest, onQuickPay, onQuickSave, onQuickCommitment, onAddLinkedPlan, onNewTracker, quickEntry = false }) {
  const {
    ...
    editGoal, deleteGoal, editGoalSaving, deleteGoalSaving,
  ...
  const confirmDeleteTracker = (item) => {
    Alert.alert(T.confirmDelete, T.confirmDeleteTracker, [
  ...
  return (
        ...
        return (
          ...
          >
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.82}
            >
              <View style={[s.amountSummary, { backgroundColor: th.cardHigh, flexDirection: rowDir }]}>
                <View style={{ flex: 1 }}>
      ...
      <View style={[s.modalOverlay, { backgroundColor: th.overlay }]}>
        <View style={[s.sheet, { backgroundColor: th.card, borderColor: th.border }]}>
          <View style={[s.sheetHeader, { flexDirection: rowDir }]}>
            <TouchableOpacity onPress={closeTrackerEdit} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
    ...
    <Modal visible={!!editPaymentDraft} transparent animationType="slide" onRequestClose={closePaymentEdit}>
      <View style={[s.modalOverlay, { backgroundColor: th.overlay }]}>
        <View style={[s.sheet, { backgroundColor: th.card, borderColor: th.border }]}>
          <View style={[s.sheetHeader, { flexDirection: rowDir }]}>
            <TouchableOpacity onPress={closePaymentEdit} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
              <Ionicons name="close" size={20} color={th.sub} />
            </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  card: { borderRadius: RADIUS.lg, borderWidth: 1, padding: 9, marginBottom: 6, ...SHADOW.card },
  ...
  amountSummary: { minHeight: 52, borderRadius: RADIUS.md, paddingHorizontal: 10, paddingVertical: 6, alignItems: 'center', gap: 8 },
