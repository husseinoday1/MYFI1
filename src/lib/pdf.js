...
const brandedPdf = async (sourceUri, label = 'Report') => {
  const fileName = `${PRODUCT_FILE_PREFIX}_${label}_${pdfStamp()}.pdf`;
  const uri = `${FileSystem.documentDirectory}${fileName}`;
  await FileSystem.copyAsync({ from: sourceUri, to: uri });
  return { uri, fileName };
};

const deliverPdf = async ({ uri, fileName, delivery = 'share', dialogTitle }) => {
  if (delivery === 'save' && Platform.OS === 'android') {
    const SAF = FileSystem.StorageAccessFramework;
    const permission = await SAF.requestDirectoryPermissionsAsync();
    if (!permission?.granted || !permission.directoryUri) return { saved: false, cancelled: true };
    const target = await SAF.createFileAsync(permission.directoryUri, fileName, 'application/pdf');
    const base64 = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 });
    await FileSystem.writeAsStringAsync(target, base64, { encoding: FileSystem.EncodingType.Base64 });
    return { saved: true, uri: target };
  }
...
<body>
  <div class="hd"><h1>🌿 ${PRODUCT_NAME}</h1><p>${month.name} · ${cfg.name}</p></div>
  <div class="stats">
  ...
  <div class="footer">${PRODUCT_NAME} · ${new Date().toLocaleDateString()}</div>
</body></html>`;

  const printed = await Print.printToFileAsync({ html, base64: false });
  const output = await brandedPdf(printed.uri, 'Monthly_Report');
  await deliverPdf({ ...output, dialogTitle: `${PRODUCT_NAME} PDF` });
  return output.uri;
};
...
export const generateFinancialReportPDF = async ({
  ...
  const label = {
    ...
    generated: ar ? `أُنشئ بواسطة ${PRODUCT_NAME}` : `Generated by ${PRODUCT_NAME}`,
  };
  const fmt = (value, currency = cfg.currency) => formatMoneyNumber(Number(value || 0), currency, lang);
  ...
  const rows = [...trans]
    ...
    .map(item => {
      ...
      const transactionCurrency = String(item.walletCurrency || item.currencyCode || cfg.currency || 'IQD').toUpperCase();
      const transactionSymbol = getSymbol(transactionCurrency);
      return `<tr>
        ...
        <td class="${amount >= 0 ? 'positive' : 'negative'}">${amount >= 0 ? '+' : '-'}${fmt(Math.abs(amount), transactionCurrency)} ${escapeHtml(transactionSymbol)}</td>
      </tr>`;
    }).join('');
  ...
  const trackerTableRows = (items = []) => items.map(item => {
    ...
    const currency = String(item.currencyCode || item.currency || cfg.currency || 'IQD').toUpperCase();
    const trackerSymbol = getSymbol(currency);
    return `<tr>
      <td class="desc"><strong>${escapeHtml(item.name || '')}</strong><small>${escapeHtml(item.due || item.createdAt || '')}</small></td>
      <td>${fmt(total, currency)} ${escapeHtml(trackerSymbol)}</td>
      <td>${fmt(paid, currency)} ${escapeHtml(trackerSymbol)}</td>
      <td>${fmt(remaining, currency)} ${escapeHtml(trackerSymbol)}</td>
    </tr>`;
  }).join('');
  const owedRows = trackerTableRows(debtRows);
  const dueRows = trackerTableRows(receivableRows);
  const groupHtml = (groups = [], field = 'remaining') => groups.length
    ? groups.map(row => `${fmt(Number(row[field] || 0), row.currency)} ${escapeHtml(getSymbol(row.currency))}`).join('<br/>')
    : `0 ${escapeHtml(sym)}`;
  const debtGroupRows = Array.isArray(debtCurrencyGroups) ? debtCurrencyGroups : [];
  const receivableGroupRows = Array.isArray(receivableCurrencyGroups) ? receivableCurrencyGroups : [];
  const debtNetReliable = [...debtGroupRows, ...receivableGroupRows].every(row => String(row.currency || '').toUpperCase() === String(cfg.currency || 'IQD').toUpperCase());
  ...
  <style>
    *{box-sizing:border-box}body{margin:0;padding:30px;background:#fff;color:#132019;font-family:Arial,sans-serif;direction:${dir};font-size:13px;line-height:1.45}
    ...
    <div class="header"><div><div class="eyebrow">${label.report}</div><h1>${escapeHtml(title)}</h1><div class="period">${escapeHtml(cfg.name || '')}</div></div><div class="brand">${PRODUCT_NAME}</div></div>
    ...
    ${selected.has('debts') ? `<div class="stats">
      <div class="stat"><span>${label.debt}</span><strong>${groupHtml(debtGroupRows, 'remaining')}</strong></div>
      <div class="stat"><span>${label.due}</span><strong>${groupHtml(receivableGroupRows, 'remaining')}</strong></div>
      <div class="stat"><span>${label.remaining}</span><strong>${debtNetReliable ? `${fmt(Number(debts.remaining || 0) - Number(receivables.remaining || 0))} ${escapeHtml(sym)}` : '—'}</strong></div>
    </div>
  ...
  const printed = await Print.printToFileAsync({ html, base64: false });
  const output = await brandedPdf(printed.uri, 'Financial_Report');
  await deliverPdf({ ...output, delivery, dialogTitle: `${PRODUCT_NAME} PDF` });
  return output.uri;
};
...
export const generateComparisonPDF = async ({ title, periods = [], cfg = {} }) => {
  ...
  const html = `<!DOCTYPE html><html dir="${dir}"><head><meta charset="UTF-8"/><style>
    *{box-sizing:border-box}body{margin:0;padding:36px;color:#132019;font-family:Arial,sans-serif;direction:${dir}}
    ...
    <div class="header"><h1>${escapeHtml(title)}</h1><div class="brand">${PRODUCT_NAME}</div></div>
  ...
  const printed = await Print.printToFileAsync({ html, base64: false });
  const output = await brandedPdf(printed.uri, 'Comparison');
  await deliverPdf({ ...output, dialogTitle: ar ? `مقارنة ${PRODUCT_NAME}` : `${PRODUCT_NAME} comparison` });
  return output.uri;
};
