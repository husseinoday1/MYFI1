...
export const generateMonthPDF = async (month, cats, cfg) => {
  ...
  const rows = [...(month.trans || [])]
    ...
    .map(t => {
      ...
      const color = t.amt > 0 ? '#4ade80' : '#fc8181';
      const sign  = t.amt > 0 ? '+' : '';
      return `
        <tr>
          <td style="color:${color};font-weight:700">${sign}${Math.abs(t.amt).toLocaleString()} ${sym}</td>
          <td>${t.dateISO || ''}</td>
...
export const generateFinancialReportPDF = async ({
  ...
  const categoryRows = topCategories.slice(0, 6).map(item => `
    <div class="category">
      ...
      <div class="track"><i style="width:${Math.min(100, Number(item.percent || 0))}%;background:${escapeHtml(item.color || '#169B62')}"></i></div>
  ...
  <style>
    *{box-sizing:border-box}body{margin:0;padding:34px;background:#fff;color:#132019;font-family:Arial,sans-serif;direction:${dir}}
    .header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #169B62;padding-bottom:18px;margin-bottom:22px}
    .brand{color:#169B62;font-size:28px;font-weight:900}.eyebrow{font-size:11px;color:#69766f;font-weight:700;letter-spacing:.4px;text-transform:uppercase}
    h1{font-size:20px;margin:5px 0 0}.period{color:#69766f;font-size:12px;margin-top:6px}
    .stats{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px}.stat{border:1px solid #e1e8e4;border-radius:12px;padding:14px}
    .stat span{display:block;color:#69766f;font-size:11px;margin-bottom:5px}.stat strong{font-size:17px}.positive{color:#118653}.negative{color:#c9565e}
    .secondary{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:22px}.secondary .stat{background:#f5f8f6}
    h2{font-size:14px;margin:22px 0 10px}.category{margin-bottom:10px}.category>div:first-child{display:flex;justify-content:space-between;font-size:12px}
    ...
    table{width:100%;border-collapse:collapse;font-size:11px}th{background:#edf7f1;color:#16784e;text-align:${ar ? 'right' : 'left'}th,td{padding:9px;border-bottom:1px solid #e7ece9}
    ...
    ${selected.has('debts') ? `<div class="secondary">
    ...
    ${selected.has('transactions') ? `<h2>${label.transactions}</h2><table><thead><tr><th>${label.date}</th><th>${label.description}</th><th>${label.category}</th><th>${label.amount}</th></tr></thead><tbody>${rows || `<tr><td colspan="4">${label.noData}</td></tr>`}</tbody></table>` : ''}
...
export const generateComparisonPDF = async ({ title, periods = [], cfg = {} }) => {
  ...
  const html = `<!DOCTYPE html><html dir="${dir}"><head><meta charset="UTF-8"/><style>
    *{box-sizing:border-box}body{margin:0;padding:36px;color:#132019;font-family:Arial,sans-serif;direction:${dir}}
    .header{display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #169B62;padding-bottom:18px;margin-bottom:24px}
    h1{font-size:20px;margin:0}.brand{color:#169B62;font-size:28px;font-weight:900}
