...
export const buildDecisionItems = ({
  ...
  const debtOn = notif.debt?.on !== false;
  const items = [];
  ...
  } else if (snapshot.forecast.status === 'warning') {
    push({
      id: 'forecast-warning',
  ...
  if (modules.budgets) {
    ...
    const urgentBudget = budgetRows.find(row => row.status === 'over')
      || budgetRows.find(row => row.status === 'near');
    if (urgentBudget) {
  ...
  const recurringDue = modules.recurring
    ? getUpcomingRecurring(trans, date).filter(item => item.daysUntil <= 3)
    : [];
  if (recurringDue.length > 0) {
    ...
    push({
      ...
      priority: 24,
      channel: first.daysUntil <= 1 ? 'important' : 'quiet',
      notify: first.daysUntil <= 1,
      throttleHours: 12,
  ...
  if (modules.debtsOwed && debtOn && snapshot.debts.remaining > 0) {
    push({
      id: 'debt-reminder',
  ...
  if (intelligence.topLeak) {
    push({
      ...
      tone: 'warning',
      priority: 28,
      channel: shouldNotifyLeak(intelligence) ? 'important' : 'quiet',
      notify: shouldNotifyLeak(intelligence),
      throttleHours: 24,
      action: { type: 'open_tab', tab: 'reports' },
    });
  }
