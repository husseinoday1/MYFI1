...
import { inspectBackupData, MAALFLOW_BACKUP_DATA_VERSION } from './backupData';
...
export const MAALFLOW_SCHEMA_VERSION = 1;
export const MAALFLOW_FORMAT = 'MAALFLOW';
...
const readPickedPackageBase64 = async (asset) => {
  ...
  if (FileSystem.cacheDirectory && asset?.uri) {
    const target = `${FileSystem.cacheDirectory}maalflow-import-${Date.now()}.zip`;
    try {
...
const inspectPackagedBackupData = (data = {}) => {
  if (data?.kind !== 'maalflow_canonical_financial_backup') return inspectBackupData(data);
...
export const buildMaalFlowPackage = async ({
  kind = 'full_backup',
  data = {},
  year = null,
  label = PRODUCT_NAME,
  ...
  const payload = {
    format: MAALFLOW_FORMAT,
    schemaVersion: MAALFLOW_SCHEMA_VERSION,
    kind,
    createdAt,
    appVersion: '1.0.0',
  ...
  const aad = `${MAALFLOW_FORMAT}:${kind}:${MAALFLOW_SCHEMA_VERSION}`;
  ...
  const bagInfo = [
    ...
    `External-Identifier: ${MAALFLOW_FORMAT}-${kind}-${createdAt}`,
    `Payload-Oxum: ${Object.values(payloadFiles).reduce((sum, file) => sum + file.length, 0)}.${Object.keys(payloadFiles).length}`,
    `MaalFlow-Kind: ${kind}`,
    `MaalFlow-Schema-Version: ${MAALFLOW_SCHEMA_VERSION}`,
    `MaalFlow-Encrypted: ${encrypted ? 'true' : 'false'}`,
    year ? `MaalFlow-Year: ${year}` : null,
    '',
...
export const inspectMaalFlowPackage = async (base64, { password = '' } = {}) => {
  const normalizedBase64 = cleanBase64(base64);
  if (String(normalizedBase64 || '').length > 70_000_000) throw new Error('MaalFlow package is too large');
  let files = null;
  try {
  ...
  if (uncompressedSize > 100_000_000) throw new Error('MaalFlow package expands beyond the safety limit');
  ...
  required.forEach(path => {
    if (!files[path]) throw new Error(`MaalFlow package is missing ${path}`);
  });
  ...
  if (encrypted) {
    ...
    if (
      encryptedMeta?.format !== MAALFLOW_FORMAT
      || encryptedMeta?.encrypted !== true
      || !encryptedMeta?.box
    ) {
      throw new Error('Invalid encrypted MaalFlow package');
    }
    if (!String(password || '')) {
      return {
        payload: null,
    ...
    try {
      const decrypted = await decryptStringWithPassword(
        encryptedMeta.box,
        password,
        encryptedMeta.aad || `${MAALFLOW_FORMAT}:${encryptedMeta.kind}:${encryptedMeta.schemaVersion}`,
      );
  ...
  if (payload?.format !== MAALFLOW_FORMAT) throw new Error('This is not a MaalFlow package');
  if (Number(payload.schemaVersion || 0) > MAALFLOW_SCHEMA_VERSION) {
    throw new Error('This backup was created by a newer MaalFlow version');
  }
  if (!payload?.data || typeof payload.data !== 'object' || Array.isArray(payload.data)) {
    throw new Error('MaalFlow package has no valid backup data');
  }
  const canonical = payload.data.kind === 'maalflow_canonical_financial_backup';
  const dataVersion = canonical
    ? Number(payload.data.manifest?.dataVersion || 0)
    : Number(payload.data.v || 0);
  if (!canonical && dataVersion > MAALFLOW_BACKUP_DATA_VERSION) {
    throw new Error('This backup data was created by a newer MaalFlow version');
  }
...
export const shareMaalFlowPackage = async ({ uri, kind = 'full_backup' } = {}) => {
  if (!uri) throw new Error('MaalFlow package file is unavailable');
...
export const saveMaalFlowPackageToDevice = async ({ base64, fileName, uri, kind = 'year_archive' } = {}) => {
  if (Platform.OS !== 'android') {
    // iOS exposes "Save to Files" through the native share sheet.
    await shareMaalFlowPackage({ uri, kind });
  ...
  if (!sourceBase64) throw new Error('MaalFlow package content is unavailable');
...
export const exportMaalFlowPackage = async (options = {}) => {
  const { delivery = 'share', ...buildOptions } = options;
  const built = await buildMaalFlowPackage(buildOptions);
  ...
  if (delivery === 'share') await shareMaalFlowPackage({ uri, kind: buildOptions.kind });
  if (delivery === 'save') await saveMaalFlowPackageToDevice({ ...built, uri, kind: buildOptions.kind });
  return { ...built, uri };
};

export const pickMaalFlowPackage = async ({ kind = null, password = '' } = {}) => {
  ...
  if (Number(asset.size || 0) > 50_000_000) throw new Error('MaalFlow package exceeds the 50 MB import limit');
  const base64 = await readPickedPackageBase64(asset);
  const inspected = await inspectMaalFlowPackage(base64, { password });
...
export const unlockMaalFlowPackage = async (picked, password, kind = null) => {
  if (!picked?.base64) throw new Error('Backup file is unavailable');
  const inspected = await inspectMaalFlowPackage(picked.base64, { password });
  const receivedKind = inspected.payload?.kind || inspected.kind;
  if (kind && receivedKind !== kind) {
    throw new Error(`Expected ${kind}, received ${receivedKind}`);
  }
