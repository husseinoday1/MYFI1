...
export const MAALFLOW_FLOW_VERSION = 1;
export const MAALFLOW_FLOW_BUCKETS = ['needs', 'wants', 'savings', 'debt', 'investment'];
...
const normalizedPercentages = (allocations = {}) => {
  ...
  const values = Object.fromEntries(MAALFLOW_FLOW_BUCKETS.map(key => [
    key,
    Math.max(0, Math.min(100, Math.round(Number(source[key]) || 0))),
  ]));
  const total = MAALFLOW_FLOW_BUCKETS.reduce((sum, key) => sum + values[key], 0);
...
const normalizedBindings = (bindings = {}, validCategoryIds = null) => {
  ...
  return Object.fromEntries(MAALFLOW_FLOW_BUCKETS.map(bucket => {
    const seen = new Set();
...
export const normalizeMaalFlowFlowPlan = (plan = {}, { categoryIds = [] } = {}) => {
...
export const buildMaalFlowFlowPreview = ({
  income = 0,
  allocations = {},
  categoryBindings = {},
  categories = [],
  ...
  const amounts = Object.fromEntries(MAALFLOW_FLOW_BUCKETS.map(bucket => [
    bucket,
  ...
  const unboundBuckets = MAALFLOW_FLOW_BUCKETS.filter(bucket => amounts[bucket] > 0 && bindings[bucket].length === 0);
...
export const buildMaalFlowFlowSavePlan = ({
  strategy = 'custom',
  income = 0,
  allocations = {},
  categoryBindings = {},
