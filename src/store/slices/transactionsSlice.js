...
import { getDefaultWalletId, getWalletAvailableBalances } from '../../lib/wallets';
...
// Money leaving a wallet (lending cash out, paying down a debt, reserving a saving) can
// never exceed what's actually available in that wallet — mirrors the same guard addTransfer
// already applies. Returns the wallet's availableBalance, or null if the wallet isn't found.
const availableInWallet = (get, walletId) => {
  const wallet = getWalletAvailableBalances(
    get().wallets,
    get().trans,
    get().cfg.currency,
    get().cfg.defaultWalletId,
  ).find(w => w.id === walletId);
  return wallet ? Number(wallet.availableBalance || 0) : null;
};
...
export const createTrackersSlice = (set, get) => ({
  addDebt: async (d) => {
    ...
    if (originMode === 'lent') {
      const available = availableInWallet(get, walletId);
      if (available === null || Math.abs(Number(debt.total) || 0) > available + 0.0001) return null;
    }
  ...
  payDebt: async (debtId, amt, dateISO = today(), walletId = null, meta = {}) => {
    ...
    if (!isReceivable) {
      // Paying down a debt you owe pulls real cash out of the wallet — can't pay more than it has.
      const available = availableInWallet(get, txWalletId);
      if (available === null || n > available + 0.0001) return false;
    }
  ...
  saveGoal: async (goalId, amt, dateISO = today(), walletId = null, meta = {}) => {
    ...
    if (n <= 0) return false;
    // Saving reserves cash in the wallet (via availableBalance) — can't reserve more than is available.
    const available = availableInWallet(get, txWalletId);
    if (available === null || n > available + 0.0001) return false;
