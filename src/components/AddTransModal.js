...
import { View, Text, Modal, TextInput, ScrollView, Alert, StyleSheet, Image, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function AddTransModal({
  ...
  const cleanInitialMode = initialMode;
  ...
  const transferSourceWallet = walletList.find(wallet => wallet.id === fromWalletId);
  const transferTargetWallets = walletList.filter(wallet => (
    wallet.id !== fromWalletId && wallet.scope === transferSourceWallet?.scope
  ));
  const canTransfer = walletList.some((wallet, index) => (
    walletList.some((candidate, candidateIndex) => candidateIndex !== index && candidate.scope === wallet.scope)
  ));
        ...
        <View style={[s.sheet, { backgroundColor: th.card, maxHeight: '88%', paddingBottom: 20 + Math.max(insets.bottom, 8) }]}>
          ...
          >
            ...
            <View style={[s.headRow, { flexDirection: rowDir }]}>
              <TouchableOpacity onPress={handleClose} style={[s.closeBtn, { backgroundColor: th.cardHigh }]}>
                <Ionicons name="close" size={20} color={th.sub} />
              </TouchableOpacity>
              <Text style={[s.title, { color: th.text, textAlign: 'center' }]}>{isEdit ? L.editTrans : focusedEntry ? focusedTitle : ''}</Text>
              <View style={{ width: 20 }} />
            </View>

            {!isEdit && !isContextualPlanningLaunch && !focusedEntry && (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.typeRow, { backgroundColor: th.cardHigh, flexDirection: rowDir }]}>
                {seg.map(sg => (
                  <TouchableOpacity
                    key={sg.k}
                    onPress={() => {
            ...
            {!isEdit && isPlanningAction && !isContextualPlanningLaunch && !focusedEntry && (
            ...
            {!isEdit && isMoneyEntry && (!focusedEntry || smartOpen) && (
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          const nextTarget = walletList.find(candidate => candidate.id !== wallet.id && candidate.scope === wallet.scope);
...
const s = StyleSheet.create({
  ...
  typeBtn:    { flex: 1, minHeight: 44, paddingHorizontal: 8, paddingVertical: 10, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  typeBtnEntry: { flex: 0, minWidth: 92 },
  ...
  moneyFields:{ borderRadius: RADIUS.lg, borderWidth: 1, padding: 12, marginBottom: 12 },
  ...
  amountInput:{ minHeight: 54, borderRadius: 14, paddingHorizontal: 14, paddingVertical: 11, borderWidth: 0.5, marginBottom: 10, fontSize: 22, lineHeight: 28, ...weight('900') },
  ...
  smartBox:   { borderWidth: 0.5, borderRadius: 14, padding: 10, marginBottom: 10 },
  smartBoxCompact:{ borderWidth: 0.5, borderRadius: 14, marginBottom: 10 },
  smartToolbar:{ alignItems: 'stretch', gap: 7 },
  smartModeBtn:{ flex: 1, minHeight: 58, borderRadius: 12, borderWidth: 1, alignItems: 'center', justifyContent: 'center', gap: 4, paddingHorizontal: 4 },
