...
import { filterByActiveScope, filterFeatureEntities, transactionFeatureEnabled } from '../lib/modules';
...
export default function HomeCenterModal({ visible, mode = 'profile', onClose, onMode, onOpenTab, onEditTransaction }) {
  const {
  ...
  const searchRows = useMemo(() => {
    ...
    const rows = [
      ...scopedTrans.map(item => ({
        ...
        resultAmount: item.kind === 'transfer' ? item.transferAmount : item.amt,
      })),
  ...
  const reviewSmart = async item => {
    ...
    onClose?.();
    onEditTransaction?.({ ...item, smartReviewedAt: item.smartReviewedAt || new Date().toISOString() });
  };
  ...
  const syncText = syncConflict ? L.conflict : !online ? L.offline : dirty ? L.pending : user ? L.synced : L.local;
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <TouchableOpacity style={[s.overlay, { backgroundColor: th.overlay }]} activeOpacity={1} onPress={onClose}>
        <TouchableOpacity activeOpacity={1} style={[s.sheet, { backgroundColor: th.card, paddingBottom: 14 + Math.max(insets.bottom, 8) }]}>
          ...
          {mode === 'search' ? (
            <>
              <View style={[s.searchBox, { backgroundColor: th.cardHigh, borderColor: th.border, flexDirection: dir }]}>
                ...
                />
                {query ? <TouchableOpacity onPress={() => setQuery('')}><Ionicons name="close-circle" size={17} color={th.faint} /></TouchableOpacity> : null}
              </View>
