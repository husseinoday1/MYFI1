...
import { PageIntro, Touchable } from '../components/AppPrimitives';
...
export default function CommitScreen({ sub, setSub, onQuickPay, onQuickSave }) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: th.bg }} contentContainerStyle={{ paddingHorizontal: 18, paddingTop: 16, paddingBottom: 120 }}>
      <PageIntro
        th={th}
        lang={cfg.lang}
        icon="card-outline"
        title={cfg.lang === 'ar' ? 'الديون والأهداف' : 'Debts and goals'}
      ...
      {modes.length > 1 && (
        <View style={[s.segment, { backgroundColor: th.cardHigh, flexDirection: isAr ? 'row-reverse' : 'row' }]}>
          {modes.includes('debt') && (
            <Touchable onPress={() => setSub('debt')}
              style={[s.segBtn, { backgroundColor: activeSub === 'debt' ? th.card : 'transparent' }]}>
              <Ionicons name="card-outline" size={14} color={activeSub === 'debt' ? th.exp : th.sub} />
              <Text style={{ color: activeSub === 'debt' ? th.exp : th.sub, ...weight('700'), fontSize: 12 }}> {L.debts}</Text>
            </Touchable>
          )}
          {modes.includes('receivable') && (
            <Touchable onPress={() => setSub('receivable')}
              style={[s.segBtn, { backgroundColor: activeSub === 'receivable' ? th.card : 'transparent' }]}>
              <Ionicons name="cash-outline" size={14} color={activeSub === 'receivable' ? th.inc : th.sub} />
              <Text style={{ color: activeSub === 'receivable' ? th.inc : th.sub, ...weight('700'), fontSize: 12 }}> {cfg.lang === 'ar' ? 'ديون لي' : 'Owed to me'}</Text>
            </Touchable>
          )}
          {modes.includes('goal') && (
            <Touchable onPress={() => setSub('goal')}
              style={[s.segBtn, { backgroundColor: activeSub === 'goal' ? th.card : 'transparent' }]}>
...
const s = StyleSheet.create({
  ...
  segBtn:  { flex: 1, minHeight: 40, flexDirection: 'row', paddingVertical: 10, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
});
