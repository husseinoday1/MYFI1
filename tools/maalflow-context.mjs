#!/usr/bin/env node
/**
 * MaalFlow deterministic context collector.
 *
 ...
 *
 * Usage:
 *   node tools/maalflow-context.mjs            # compact baseline + staleness report
 *   node tools/maalflow-context.mjs --fetch    # same, after `git fetch --prune`
 *   node tools/maalflow-context.mjs --json     # machine-readable
 *
...
const DOMAINS = [
  ...
  [/^src\/lib\/financialRestore|^src\/lib\/maalflowFiles|backup/i, 'backup-restore'],
  ...
  [/^docs\/01_CORE_AUTHORITY\/|^docs\/00_MAALFLOW_CANONICAL_AUTHORITY/, 'planning-authority'],
...
const states = {
  'PROJECT_STATE': provenanceHead('.maalflow-ai/PROJECT_STATE.md'),
  'CURRENT_TASK': provenanceHead('.maalflow-ai/CURRENT_TASK.md'),
};
...
const L = [];
L.push('MaalFlow CONTEXT BASELINE  (evidence only — draws no conclusions)');
